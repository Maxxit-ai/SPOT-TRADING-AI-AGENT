...
# Connect to the server
sio.connect('http://localhost:3006')
```
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        throw new Error(
          `Signal processing failed after ${maxRetries} attempts`
        );
      }
...
client.on("messageCreate", async (message) => {
  ...
  if (message.content.startsWith("!signal")) {
    try {
      ...
      message.reply(
        `✅ Signal processed successfully! ID: ${response.data.signalId}`
      );
    } catch (error) {
      ...
      setInterval(async () => {
        try {
          const response = await fetch(
            "http://localhost:3006/api/signal/status"
          );
...
async function loadTest(concurrency = 10, totalRequests = 100) {
  ...
  for (let i = 0; i < totalRequests; i++) {
    ...
    requests.push(
      axios.post("http://localhost:3006/api/signal/process", signal)
    );
  }
